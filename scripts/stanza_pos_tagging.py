#!/usr/bin/env python3
"""Stanza POS tagging for Tatoeba Italian sentences.

This is a one-off preprocessing script that generates POS-tagged tokens for
Tatoeba sentences. The output JSONL is then imported into the database by
the sentence_tokens importer.

Usage:
    # Tag all Italian sentences (slow, ~1-2 hours for 377K sentences)
    python scripts/stanza_pos_tagging.py

    # Tag only CK-whitelisted sentences (recommended for initial testing)
...
# File paths
DEFAULT_INPUT = Path("data/tatoeba/ita_sentences.tsv")
DEFAULT_OUTPUT = Path("data/tatoeba/ita_sentences_pos.jsonl")
...
def parse_sentences_tsv(path: Path) -> dict[int, str]:
    """Parse Tatoeba sentences TSV file.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Stanza POS tagging on Tatoeba Italian sentences",
    ...
    parser.add_argument(
        ...
        help=f"Path to Italian sentences TSV (default: {DEFAULT_INPUT})",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--ck-only",
        action="store_true",
        help="Only process sentences with CK-whitelisted English translations",
    )
    parser.add_argument(
        "--sentences-in-lists",
        type=Path,
    ...
    sentences = parse_sentences_tsv(args.input)
    print(f"  Loaded {len(sentences):,} sentences")

    # Filter to CK-whitelisted if requested
    if args.ck_only:
    ...
    run_pos_tagging(sentences, args.output, batch_size=args.batch_size)
