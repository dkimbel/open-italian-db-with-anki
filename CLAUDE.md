...
- `task import-morphit && task import-itwac` - Enrichment
- `task generate-deck` - Create `output/italian_verbs.apkg`
- `task preview-card VERB=parlare` - Preview card in browser

### Data Exploration

DuckDB sessions for exploring raw source data: `task wikt-query`, `task tatoeba-query`, `task morphit-query`, `task itwac-query`, `task partut-query`, `task opensub-query`, `task paisa-query`, `task data-query` (all sources).
